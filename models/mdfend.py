...
class MultiDomainFENDModel(torch.nn.Module):
    def __init__(self, emb_dim, mlp_dims, bert, dropout, emb_type):
        ...
        expert = [
            cnn_extractor(feature_kernel, emb_dim) for _ in range(self.num_expert)
        ]
    ...
    def forward(self, **kwargs):
        ...
        idxs = torch.tensor(list(category)).view(-1, 1).cuda()
...
class Trainer:
    ...
    def train(self, logger=None):
        ...
        self.model = MultiDomainFENDModel(
            self.emb_dim, self.mlp_dims, self.bert, self.dropout, self.emb_type
        )
        if self.use_cuda:
        ...
        self.model.load_state_dict(
            torch.load(os.path.join(self.save_param_dir, "parameter_mdfend.pkl"))
        )
