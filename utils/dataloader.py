...
class bert_data:
    ...
    def load_data(self, path, shuffle):
        ...
        content_token_ids, content_masks = word2input(
            content, self.vocab_file, self.max_len
        )
        dataset = TensorDataset(content_token_ids, content_masks, label, category)
        return DataLoader(
            dataset=dataset,
...
class w2v_data:
    def __init__(
        self, max_len, batch_size, emb_dim, vocab_file, category_dict, num_workers=2
    ):
        self.max_len = max_len
        self.batch_size = batch_size
    ...
    def encode(self, token_ids):
        ...
        for token_id in token_ids:
            ...
            words = list(token_id[: self.max_len])
            words_vec = [
                w2v_model[word] if word in w2v_model else np.zeros([self.emb_dim])
                for word in words
            ]
            words_vec.extend(
                np.zeros([self.emb_dim]) for _ in range(len(words_vec), self.max_len)
            )
    ...
    def load_data(self, path, shuffle=False):
        ...
        dataset = TensorDataset(emb_content, content_masks, label, category)
        return DataLoader(
            dataset=dataset,
            batch_size=self.batch_size,
            num_workers=self.num_workers,
